window.bcsh = {
    commands: {
                ...
                "interval": (args)=>{
                    let iid = setInterval(()=>{ 
                        if(!eval(args[0])) clearInterval(iid); 
                        bcsh.exec(args[1]) 
                    }, args[2]); 
                    return iid },
                "func": (args)=>{ 
                    bcsh.functions[args[0]] = args[1]; 
                },
                "funclist": (args)=>{
                    return Object.keys(bcsh.functions).join(args[0]?args[0]:", ");
                },
                "f": (args)=>{
                    let code = bcsh.functions[args[0]];
                    for(let i in args) code = code.replaceAll(`@{#${i}}`, args[i]);
                ...
                "exec": (args)=>{
                    return bcsh.exec(args[0]);
                },
                "execScript": (args)=>{
                    return bcsh.execScript(args[0]);
                },
                "str.replace": (args)=>{ 
                    return (args[3]?args[0].replace(args[1], args[2]):args[0].replaceAll(args[1], args[2]));
                },
                ...
                "arr.length": (args)=>{ return JSON.parse(args[0]).length },
            ...
            exec: function(cmdto){
                    ...
                    if(!(cmd in this.commands)){
                        bcsh.error({message: `Команда "${cmd}" не найдена.`});
                        return;
                    }
                    ...
                    let found = false;
                        ...
                        pa.forEach((e,i,o)=>{
                            ...
                            if(o[i].toString().match(/@{.+}/) && o[i].toString[0] != "&"){
                                try{ 
                                    o[i] = o[i].toString().replace(/@{([^}]+)}/g, function(p1){
                                        return eval(p1.replaceAll('@{','').replaceAll('}',''));
                                    });
                                }catch(e){
                                    bcsh.error(e);
                                    ret = e; 
            ...
            error: function(obj){
                obj.fatal = !!obj.fatal;
                window.errorData = JSON.stringify(obj);
                let msg = `[${this.debugData.command}:${this.debugData.line}] >> ${obj.message}`;

                switch(obj.fatal?this.options.fatalErrorAction:this.options.errorAction){
                    case "ignore": break;
                    case "default": {
                        console.error(msg);
                        break;
            ...
            execScript: function(code){
                ...
                code = code.split("\n");
                for(let i in code){
                    this.debugData.line = i+1;
                    if(code[i].startsWith("#") || code[i] == "") continue;
                    this.exec(code[i]);
                }
                this.debugData.line = 1;
            },
            options: {
                errorAction: "default",
                fatalErrorAction: "stop"
            },
            debugData: {
                command: "",
                line: 1
